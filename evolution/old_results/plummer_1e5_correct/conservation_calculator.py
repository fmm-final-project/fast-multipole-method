...
def calculate_potential_energy_gpu(filename="uniform_sphere_3d_1e6.bin", n_particles=1_000_000):
    ...
    return total_potential_energy[0], total_kinetic_energy[0], total_momentum_x[0], total_momentum_y[0], total_momentum_z[0]
...
if __name__ == "__main__":
    ...
    for i in range(501):
        datafile = f'particles/plummer_vel_3d_1e5_step{i}.bin'
        U, K, Px, Py, Pz =calculate_potential_energy_gpu(filename=datafile, n_particles=100_000)
        summary.append({
            "i": i,
            "U": U,
            "K": K,
    ...
    df_summary.to_csv("conservation/conservation_results.csv", index=False)
    print(df_summary)