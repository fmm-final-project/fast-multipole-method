...
def var_vel(infile='particles/plummer_vel_3d_1e5_step0.bin', outfile='particles/plummer_vel_3d_1e5_step0_var.bin'):
    ...
    var_vel = particles[:, 4:7] * 0.8   # 速度 (vx, vy, vz) * 0.9 => 測試Jeans theorem
    print(f"成功讀取 {len(particles)} 顆粒子。")

    updated_particles = np.hstack((mass, pos, var_vel))
    updated_particles.tofile(outfile)
    print(f"成功更改速度資訊至 {outfile} 。")


def run_KDK_step(dt=0.01, exe_name=exe_name, method=method, infile='particles/plummer_vel_3d_1e5_step0.bin', outfile='particles/plummer_vel_3d_1e5_step1.bin'):
    """
    執行一步 KDK 軌道積分。
    """
    # --- 讀取資料 ---
    ...
    if os.path.exists(f'particles_{method}/half_step.bin'):
        os.remove(f'particles_{method}/half_step.bin')
