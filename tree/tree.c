...
void evaluateForceOnParticle(Particle* p, Cell* cell){
    if(cell->nParticles > 0){
        // Direct summation
        for(int i = 0; i < cell->nParticles; i++){
        ...
        if(cell->size < r * THETA){
            ...
            for(int c = 0; c < 8; c++){
                if(cell->children[c]) evaluateForceOnParticle(p, cell->children[c]);
            }
        }
    }
}
...
int main(){
    ...
    */
    ...
    size_t written = fwrite(force, sizeof(double), 3 * N, fbin);
    if(written != 3 * N){
